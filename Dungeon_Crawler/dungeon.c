...
int gen_dungeon(dungeon_t *d)
{
  ...
  // upstairs
  int staircase_y = rand_range(d->rooms->position[dim_y],
                                     (d->rooms->position[dim_y] +
                                      d->rooms->size[dim_y] - 1));
  int staircase_x = rand_range(d->rooms->position[dim_x],
                                     (d->rooms->position[dim_x] +
                                      d->rooms->size[dim_x] - 1));
  d->map[staircase_y][staircase_x] = ter_staircase_up;
  
  // downstairs
  staircase_y = rand_range(d->rooms->position[dim_y],
                                     (d->rooms->position[dim_y] +
                                      d->rooms->size[dim_y] - 1));
  staircase_x = rand_range(d->rooms->position[dim_x],
                                     (d->rooms->position[dim_x] +
                                      d->rooms->size[dim_x] - 1));
  d->map[staircase_y][staircase_x] = ter_staircase_down;
	...
	case ter_staircase_down:
	  ...
	  refresh();
        ...
        case ter_debug:
          mvprintw(p[dim_y], p[dim_x], "%c", '*');
	  refresh();
          //mvprintw(win, 0, 0, "Debug character at %d, %d\n", p[dim_y], p[dim_x]);
          break;
        }
      }
    }
